...
CREATE TABLE `chatroom_online_users` (
  ...
  `status` enum('online','idle','away','busy') DEFAULT 'online' COMMENT '在线状态',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_online_chatroom_user` (`chatroom_id`,`user_id`) COMMENT '用户在同一聊天室中的在线记录唯一',
...
CREATE TABLE `chatroom_settings` (
  ...
  `updated_at` timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_setting_chatroom_key` (`chatroom_id`,`setting_key`) COMMENT '同一聊天室的设置键唯一',
...
CREATE TABLE `chatroom_message_reads` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_read_message_user` (`message_id`,`user_id`) COMMENT '用户对同一消息的阅读记录唯一',
...
LEFT JOIN (
    SELECT 
        ...
        COUNT(*) as message_count
    FROM chatroom_messages
    WHERE status = 'normal' AND user_id IS NOT NULL
...
FROM chatroom_messages
WHERE status = 'normal' AND created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
GROUP BY DATE(created_at), chatroom_id, message_type
ORDER BY message_date DESC, chatroom_id;
...
-- 完成消息
-- =============================================
SELECT 'Chatroom database structure created successfully!' as Result;
